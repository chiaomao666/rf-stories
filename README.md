...
UW 劇情文本會把抓取者的玩家名寫進 `speaker` 與 `dialogue`（實測 93 張中有 40 句）。
`uw_probe.py` 會在寫檔前自動替換成「主角」，但**公開任何資料前請再確認一次**——
帶著玩家名的文本等同散佈帳號身分。主線劇情目前沒有觀察到這個情形。

## 資料與版控

`data/` 預設不進版控：內容是從遊戲伺服器抓下來的原始素材路徑與文本，
且可能殘留帳號相關資訊。要公開發佈時，走一次清理與素材鏡像再另行處理。
