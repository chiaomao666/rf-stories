...
> | 資料集 | 帳號暱稱 | 出現處數 |
> |---|---|---|
> | 主線 非紅軍（8,867 張） | `中華電信` | 1,929 |
> | 主線 紅軍（8,289 張） | `_` | 3,253 |
> | UW site 666（93 張） | `中華電信` | 40 |
>
> `speaker` 是整格相等，`dialogue` 則是內嵌（例：「主角走向站在路旁的粉圓。」），
> 兩者都要處理。暱稱本身可能是任意字串——test06 的暱稱就是單一底線 `_`，
> 代入對白後與一般標點難以分辨，所以務必以帳號的實際暱稱做替換，不要靠樣式猜測。
>
> **抓到的文本等同帶著抓取帳號的身分。** 公開前必須把玩家名以佔位符取代，
> 否則等於將帳號名散佈出去。這是上線前的阻斷性項目，不是選配的清理步驟。
>
> 本 repo 的 `fetch_main_story.py` 與 `uw_probe.py` 會在寫檔前自動替換為「主角」，
