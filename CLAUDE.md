...
2. **玩家名去識別化是入庫前的硬性關卡**。伺服器把登入帳號暱稱直接代入 `speaker` 與 `dialogue`
   （實測非紅軍 1,929 處、紅軍 3,253 處）。抓取時自動替換為「主角」，既有資料用 `scrub_data.py` 補做。
   暱稱可能是任意字串（有測試帳號的暱稱就是單一底線 `_`），**只能用帳號實際暱稱替換，不可用樣式猜測**。
   未清理的原始 dump 不得進版控。
